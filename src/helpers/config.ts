...
export async function getPersonalAgentConfig(context: Context, owner: string) {
  ...
  if (config) {
    try {
      const configSchemaWithDefaults = Value.Default(personalAgentConfigSchema, config) as Readonly<unknown>;
      ...
      return { config: Value.Decode(personalAgentConfigSchema, configSchemaWithDefaults), errors, rawData };
    } catch (error) {
